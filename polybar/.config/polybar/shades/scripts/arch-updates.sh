...
path=${HOME}/.config/polybar/shades/scripts/arch/
#trap 'exit' SIGINT
  ...
  check_for_updates() {
    ...
    sudo pacman  -Syu  | nl -w2 -s '. ' | sed 's/://' >| ${path}aur.pkgs
  ...
  check_for_vcs_updates() {
    yay vercmp-devel | sed 's/://' | nl -w2 -s '. ' >| ${path}aur-vcs.pkgs
  ...
  upgrade() {
    if [ -s ${path}repo.pkgs ]; then
      [ -s ${path}aur.pkgs ] && urxvtc -tr  -sh 20 -fg white -bg black -e sh -c "aur sync -c -u --noview && sudo pacman -Syu --noconfirm" || \
                                urxvtc -tr  -sh 20 -fg white -bg black -e sh -c "sudo pacman -Syu --noconfirm"
  ...
  vcs_upgrade() {
    ...
    [ -s ${path}aur-vcs.pkgs ] && urxvtc -tr -sh 20 -fg white -bg black -e bash -ci \
                                  'mapfile -t packages < <(yay vercmp-devel | cut -d: -f1) && yay sync "${packages[@]}" --no-ver-shallow'
